...
- Sends `Ack` frames on a dual-track schedule: the `in_memory_sequence` field advances as soon as a batch is received and published into the receiver's input ring (pre-journal), while `acked_sequence` advances only once the journal stage confirms durable write (cursor advance). Each iteration of the receive loop emits at most one ack carrying the latest value of both fields, so the primary's policy can gate on either level without a separate ack stream. Acks are pipelined: up to 8 batches can be submitted to the journal stage before the first durable ack is sent, overlapping NVMe writes with TCP receives.
