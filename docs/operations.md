...
> **Upgrading from a release before 0.15:** this flag was `--durability-mode`, and its values were `local`, `replicated`, `hybrid` and `durably-replicated`. It is now `--ack-policy` with `disk`, `ram`, `disk+ram` and `two-disks` respectively. The guarantee behind each is unchanged — only the names are — but a server started with the old flag will refuse to launch, so update your unit files and launch scripts before rolling out.
