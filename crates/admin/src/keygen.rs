...
fn main() {
    ...
    if args.len() < 3 {
        ...
        eprintln!("  permission: admin | trader | custodian | readonly");
        eprintln!();
        eprintln!("example:");
    ...
    match permission.as_str() {
        "admin" | "trader" | "custodian" | "readonly" => {}
        other => {
            eprintln!("error: invalid permission '{other}' (expected admin/trader/custodian/readonly)");
            std::process::exit(1);
        }
    }
