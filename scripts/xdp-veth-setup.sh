...
echo ""
echo "  Build:   cargo build --release -p melin-ec-server -p melin-ec-bench --features melin-ec-server/synthetic-seed"
echo "  Server:  sudo target/release/melin-ec-server --xdp-interface $VETH1 --bind 10.99.0.2:9876 --journal /tmp/xdp-test.journal --accounts 10 --instruments 2"
echo "  Bench:   target/release/melin-ec-bench --addr 10.99.0.2:9876 --warmup-duration 2s --duration 10s --accounts 10 --instruments 2"
echo ""
