...
#
# The sequencer crates (melin-app, melin-client, melin-pipeline, ...) are
# published from the Melin sequencer repository, not from here; this script
# only ever uploads the `melin-ec-*` workspace members.
#
...
# `publish = false` members — so a newly added crate cannot be forgotten by a
# list nobody updated.
#
...
# of fourteen leaves four live forever, and re-running plain `--workspace` then
...
# Read with grep rather than `cargo metadata | jq` so the dry-run path needs
# neither jq nor a metadata pass.
...
# A published version that will not say what changed in it is one no operator
# can plan an upgrade around, and the entry cannot be added afterwards —
# crates.io is immutable. Checked in both modes, so a dry run catches a missing
# entry before the release rather than during it, and checked here rather than
# in release.sh so a publish started by hand is guarded too.
if [[ ! -f CHANGELOG.md ]]; then
    echo "error: CHANGELOG.md is missing; every published version needs an entry" >&2
    exit 1
fi
# Keep a Changelog headings, e.g. `## [0.16.0] - 2026-09-14`. Only `.` is a
...
# uploads, so what is on crates.io is irrelevant to it — and on a released
# version every crate *is* already published, so filtering would reduce the
# check to nothing. Verifying that every crate still packages and builds is the
# whole point. It also needs neither curl nor jq.
#
...
# `cargo info <crate>@<version>` answers from the local path ("version: 0.15.0
# (from ./crates/exchange/types)"), and during a release the local version is
# by definition the version being released — so it would report every crate as
...
echo
echo "==> Publishing"
# `--locked` so the release is built against the dependency versions the
# release gate tested rather than whatever re-resolves at publish time; the
# lockfile is the record of what was actually verified. `${EXCLUDES[@]+...}` so
# an empty array does not trip `set -u` on bash < 4.4.
